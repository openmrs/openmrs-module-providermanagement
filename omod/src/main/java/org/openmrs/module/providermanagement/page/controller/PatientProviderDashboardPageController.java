...
public class PatientProviderDashboardPageController {
    ...
    public void controller(PageModel pageModel,
        ...
        for (RelationshipType relationshipType : pmService.getAllProviderRoleRelationshipTypes(false)) {
            ...
            if (p != null && p.size() > 0) {
                providerMap.put(relationshipType,
                        ProviderManagementWebUtil.convertPersonListToSimpleObjectList(p, ui,
                                ProviderManagementGlobalProperties.GLOBAL_PROPERTY_PROVIDER_LIST_DISPLAY_FIELDS()
                                        .values().toArray(new String[0])));
            }
            // otherwise, get suggestions for this relationship type
            else {
                // note that we still still add a blank map entry to providers so that we can
                // iterate over the keys in the view
                providerMap.put(relationshipType, null);

                // get the suggestions for the patient, cnonvert them to a simple object, and
                // add them to the map
                providerSuggestionMap.put(relationshipType,
                        ProviderManagementWebUtil.convertPersonListToSimpleObjectList(
                                Context.getService(ProviderSuggestionService.class).suggestProvidersForPatient(patient,
                                        relationshipType),
                                ui, ProviderManagementGlobalProperties.GLOBAL_PROPERTY_PROVIDER_LIST_DISPLAY_FIELDS()
                                        .values().toArray(new String[0])));
            }
        }
        ...
        // add the global properties that specifies the fields to display in the
        // provider and patient field and search results
        pageModel.addAttribute("providerSearchDisplayFields",
                ProviderManagementGlobalProperties.GLOBAL_PROPERTY_PROVIDER_SEARCH_DISPLAY_FIELDS());
        pageModel.addAttribute("providerListDisplayFields",
                ProviderManagementGlobalProperties.GLOBAL_PROPERTY_PROVIDER_LIST_DISPLAY_FIELDS());
    }
