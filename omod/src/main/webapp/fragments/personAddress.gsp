    ...
    <tr>
        <td><span class="label">${ ui.message(item.displayText) }:</span></td>
          <% if (config.mode?.equalsIgnoreCase('edit')) { %>
               <td><input type="text" id="${ item.codeName }" name="${ item.codeName }" value="${ config.personAddress && config.personAddress[item.codeName] ? config.personAddress[item.codeName]  : '' }" size="${ item.displaySize }"/></td>
          <% }
